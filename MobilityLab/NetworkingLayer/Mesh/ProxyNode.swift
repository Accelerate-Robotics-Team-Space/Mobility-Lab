...
final class ProxyNode: NSObject {
    ...
    private let browserTimerInterval: TimeInterval
    ...
    // MARK: - Init
    init(peerId: MCPeerID, container: Container = .shared, interval: TimeInterval = 10) {
        ...
        self.browserTimerInterval = interval
        ...
        self.edgeBrowser = MultiPeerBrowser(for: peerId, and: .edge, container: container)
        self.proxyBrowser = MultiPeerBrowser(for: peerId, and: .proxy, container: container)
    ...
    func start() {
        edgeBrowser.start()
        startProxyBrowserTimer(with: browserTimerInterval)
    }
...
private extension ProxyNode {
    ...
    // MARK: - Timer
    func startProxyBrowserTimer(with timeInterval: TimeInterval) {
