    ...
    var body: some View {
        VStack {
            ...
            Button {
                ...
                if !activity.isPause {
                    HStack {
                        Image(activity.targetPosition.imageStr)
                            .resizable()
    ...
    static var previews: some View {
        ActivityBadge(
            activity: ActivityStartEnd(
                startDate: Date().startOfDay.addingTimeInterval(5 * 60 * 60),
                endDate: Date().startOfDay.addingTimeInterval(5.05 * 60 * 60),
                actualPosition: .left,
                targetPosition: .left,
                startTime: 5 * 60 * 60,
                endTime: 5.05 * 60 * 60,
