...
# The Firebase CICD has a lag of about an hour when publishing a new version, during which time the 
# download of 'latest' will fail. In this situation, the `latest` tag has been updated, but the new binary is not yet available
...
GH_TOKEN="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"
...
if [[ ! -z "$FIREBASE_BINARY" ]]; then
    ...
    echo "    - Firebase Tools not currently installed."
fi
...
echo "-- Checking latest releases for Firebase Tools..."
# List the 2 previous Download archives: (i.e. "v14.25.0\nv14.24.2")
curl https://api.github.com/repos/firebase/firebase-tools/releases \
--header "Authorization: Bearer $GH_TOKEN" \
--silent \
-o releases.json

if [ $? -ne 0 ]; then
    echo "curl of firebase-tools/releases failed"
fi

PREVIOUS="v14.25.0"
TWO_RELEASES_AGO="v14.24.2"

jq '.[].name' -r releases.json > releasesList.txt 2>/dev/null
if [ $? -ne 0 ]; then
    jq '.message' releases.json | sed 's/"//g' > message.txt 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "JSON Parsing Failed when checking for latest releases"
        echo "Raw JSON:"
        echo ""
        cat releases.json
    else
        echo "Error Getting Release List:"
        cat message.txt
    fi
else
    PREVIOUS=$(cat releasesList.txt | head -n 2 | tail -1)
    TWO_RELEASES_AGO=$(cat releasesList.txt | head -n 3 | tail -1)

    echo "  Current Firebase Tools Release:"
    echo "    - $(cat releasesList.txt | head -n 1)"
    echo "  Previous 2 Firebase Tools Releases:"
    echo "    - $PREVIOUS"
    echo "    - $TWO_RELEASES_AGO"
    echo ""  
fi

# We have enough information to generate the binary's download URL.
DOWNLOAD_URL="https://firebase.tools/bin/$MACHINE/latest"
# alias for: https://github.com/firebase/firebase-tools/releases/download/latest/firebase-tools-macos
...
if [ $? -ne 0 ]; then
    echo "Curl command failed for downloading latest version!"
    DOWNLOAD_URL="https://github.com/firebase/firebase-tools/releases/download/$PREVIOUS/firebase-tools-$MACHINE"
    echo "-- Downloading binary from $DOWNLOAD_URL"
    curl -o "/tmp/firebase_standalone.tmp" -L --progress-bar $DOWNLOAD_URL
    if [ $? -ne 0 ]; then
        echo "Curl command failed for downloading version $PREVIOUS!"
        DOWNLOAD_URL="https://github.com/firebase/firebase-tools/releases/download/$TWO_RELEASES_AGO/firebase-tools-$MACHINE"
        echo "-- Downloading binary from $DOWNLOAD_URL"
        curl -o "/tmp/firebase_standalone.tmp" -L --progress-bar $DOWNLOAD_URL
        if [ $? -ne 0 ]; then
            echo "Could not download Firebase binary"
            exit -1
        fi
    fi
fi

# https://github.com/firebase/firebase-tools/releases/download/v14.25.0/firebase-tools-macos
