...
class DataPoint: Identifiable {
    ...
    required init?(serialize data: Data) {
        guard let idData = data.to(type: Int64.self, from: 0),
    ...
    init(
        ...
        yGravity: Double,
        zGravity: Double,
        xRotationRate: Double,
        yRotationRate: Double,
        zRotationRate: Double,
        rollAttitude: Double,
        pitchAttitude: Double,
        yawAttitude: Double
    ) {
        self.id = id
        self.xAccel = xAccel
    ...
    func apply(_ calibration: DataPoint) -> DataPoint {
        let point = DataPoint(
            id: id,
            xAccel: xAccel,
            yAccel: yAccel,
            zAccel: zAccel,
            xGravity: xGravity,
            yGravity: yGravity, zGravity: zGravity,
            xRotationRate: xRotationRate,
            yRotationRate: yRotationRate,
            zRotationRate: zRotationRate,
            rollAttitude: rollAttitude - calibration.rollAttitude,
            pitchAttitude: pitchAttitude - calibration.pitchAttitude,
            yawAttitude: yawAttitude
        )
        return point
    }

    /// Gravity Vector of the data point
    var gravityVector: DataPoint.Vector {
        DataPoint.Vector(
            x: xGravity,
            y: yGravity,
            z: zGravity
...
extension DataPoint: Serializable {
    func toData() -> Data {
        ...
        buffer.append(Data(from: self.id.bigEndian)) // 0 - 7
        buffer.append(Data(from: self.xAccel.bitPattern.bigEndian)) // 8 - 15
        buffer.append(Data(from: self.yAccel.bitPattern.bigEndian)) // 16 - 23
        buffer.append(Data(from: self.zAccel.bitPattern.bigEndian)) // 24 - 31
        buffer.append(Data(from: self.xGravity.bitPattern.bigEndian)) // 32 - 39
        buffer.append(Data(from: self.yGravity.bitPattern.bigEndian)) // 40 - 47
        buffer.append(Data(from: self.zGravity.bitPattern.bigEndian)) // 48 - 55
        buffer.append(Data(from: self.xRotationRate.bitPattern.bigEndian)) // 56 - 63
        buffer.append(Data(from: self.yRotationRate.bitPattern.bigEndian)) // 64 - 71
        buffer.append(Data(from: self.zRotationRate.bitPattern.bigEndian)) // 72 - 79
        buffer.append(Data(from: self.rollAttitude.bitPattern.bigEndian)) // 80 - 87
        buffer.append(Data(from: self.pitchAttitude.bitPattern.bigEndian)) // 88 - 95
        buffer.append(Data(from: self.yawAttitude.bitPattern.bigEndian)) // 96 - 104

        return buffer
    }

    func toSimpleData() -> Data {
        var buffer = Data(capacity: 15)

        buffer.append(Data(from: self.rollAttitude.bitPattern.bigEndian)) // 0 - 7
        buffer.append(Data(from: self.pitchAttitude.bitPattern.bigEndian)) // 8 - 15
