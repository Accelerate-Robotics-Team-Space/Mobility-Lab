...
final class ProvisioningAPIServiceTests: XCTestCase {
    ...
    func testRegisterUMM() async throws {
        let expectation = expectation(description: "provisioningAPI-registerUMM")
        ...
        let testID = "Test ID"
        let registration = try await testSubject.registerUMM(deviceId: testID)

        await fulfillment(of: [expectation], timeout: 1)

        XCTAssertEqual(
            request?.url?.absoluteString,
            "\(NetworkingConstants.baseUrlStr)api/v1/SensorSuiteProvisioning/RegisterUnitMobilityMonitor/Test%20ID"
        )
    ...
    func testGetCRL() async throws {
        ...
        XCTAssertEqual(
            request?.url?.absoluteString,
            "\(NetworkingConstants.baseUrlStr)api/v1/SensorSuiteProvisioning/crl/\(facilityId)"
        )
